...
socketDataAR * listenerSocketAR(int port) 
{
 socketDataAR *sock;

 sock=(socketDataAR *)malloc(sizeof(socketDataAR));
...
int listenerAR(int argc, const char *argv[])
{
    socketDataAR *sock;
    sock=listenerSocketAR(39994);
    char tempNum[20],*ipaddr,temp[1024],username[20],msg[1024];
    int addrlen,i=-1;
    FILE *fp;
